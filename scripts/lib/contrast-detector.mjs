...
import { normalizeText, sentenceSpans } from './utils.mjs';
import { STAGE1_REGEXES } from './regexes-stage1.mjs';
import { STAGE2_REGEXES } from './regexes-stage2.mjs';
import { hasPosTagger, tagStreamWithOffsets } from './pos-tagger.mjs';

const DBG = !!process.env.SLOP_DEBUG;
const dbg = {
  group: (...a) => { if (DBG) console.error(...a); },
  groupEnd: () => {},
  log: (...a) => { if (DBG) console.error(...a); },
  warn: (...a) => { if (DBG) console.error(...a); },
  error: (...a) => { if (DBG) console.error(...a); },
};
...
export function extractContrastMatches(text) {
  dbg.group('🔎 Contrast Pattern Detection');
  dbg.log('Input length:', text.length, 'chars');

  const tNorm = normalizeText(text);
  const spans = sentenceSpans(tNorm);
  const candidates = [];

  dbg.log('Normalized text:', tNorm.substring(0, 200) + (tNorm.length > 200 ? '...' : ''));
  dbg.log('Sentence spans:', spans.length);
  ...
  for (const [pname, pregex] of Object.entries(STAGE1_REGEXES)) {
    ...
    if (matches.length > 0) {
      dbg.log(`  ✓ Pattern ${pname} matched ${matches.length} time(s)`);
    ...
    for (const match of matches) {
      ...
      if (rng) {
        ...
        dbg.log(`    Match: "${match[0].substring(0, 60).trim()}..." (${sentenceSpan} sent)`);
        if (sentenceSpan > 2) {
          dbg.warn(`    ⚠ Pattern ${pname} matched ${sentenceSpan} sentences!`);
          dbg.warn(`       Full match: "${match[0].substring(0, 150)}..."`);
        }
      }
    }
  }
  ...
  if (hasPosTagger() && Object.keys(STAGE2_REGEXES).length > 0) {
    ...
    dbg.group('🔍 Stage2 POS Tagging Debug');
    dbg.log('Original text:', tNorm.substring(0, 200) + (tNorm.length > 200 ? '...' : ''));
    dbg.log('Tagged stream:', stream.substring(0, 2000) + (stream.length > 200 ? '...' : ''));
    dbg.log('Number of pieces:', pieces.length);
    dbg.log('First 10 pieces:', pieces.slice(0, 10));
    dbg.groupEnd();
      ...
      for (const match of matches) {
        ...
        if (mapRes) {
          ...
          if (rng) {
            ...
            dbg.log(`    Match: "${match[0].substring(0, 60)}..." (${sentenceSpan} sent)`);
            if (sentenceSpan > 2) {
              dbg.warn(`    ⚠ Pattern ${pname} matched ${sentenceSpan} sentences!`);
              dbg.warn(`       Tagged match: "${match[0].substring(0, 150)}..."`);
              dbg.warn(`       Raw text: "${tNorm.substring(rs, re).substring(0, 150)}..."`);
            }
          }
        }
      }
  ...
  dbg.log('Candidates before merging:', candidates.length);

  const merged = mergeIntervals(candidates);
  dbg.log('Candidates after merging:', merged.length);
  ...
  for (const it of merged) {
    ...
    dbg.log(`  Result ${results.length}: ${it.pattern_name} (spans ${sentenceSpan} sentence${sentenceSpan > 1 ? 's' : ''})`);
    if (sentenceSpan > 2) {
      dbg.error(`  ❌ BUG: Pattern ${it.pattern_name} matched across ${sentenceSpan} sentences! This is too greedy.`);
      dbg.error(`     Match text: "${it.match_text}"`);
      dbg.error(`     Full sentence block: "${result.sentence.substring(0, 2000)}..."`);
    } else {
      dbg.log(`    Sentence: "${result.sentence.substring(0, 800)}..."`);
    }
  }
